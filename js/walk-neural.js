...
var ailoop = null
var NUM_HIDDEN = 10
var NN = new NeuralNetwork(14,NUM_HIDDEN,4)
var KEYMASK = [1,2,4,8]
var nn_ready = false,
    qt_ready = false
var curState = null
...
function run() {
    if (qt_ready && nn_ready) {
	   var ailoop = setInterval(iterate,50)
    } else {
        loadDatasets()
    }
}
...
function chooseAction(s) {
    ...
    for (var i = 0; i < ACTIONS.length; i++) {
        ...
        var q = Q(s,action) + NN_WEIGHT*bias[action]
        if (q > bestQ) {
            bestActions = [action]
            bestQ = q
        } else if (q === bestQ) {
...
function loadDatasets(qtable,nnetwork) {
	qtable = qtable || 'qtab-compressed'
    nnetwork = nnetwork || 'nnet-10'
